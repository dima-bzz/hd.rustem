...
class UploadHandlerComment
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
      global $CONF;
        $this->response = array();
        $this->options = array(
            'script_url' =>dirname( $this->get_full_url()).'/',
            'upload_dir' => dirname(dirname($this->get_server_var('SCRIPT_FILENAME'))).'/upload_files/',
            'upload_url' =>dirname( $this->get_full_url()).'/upload_files/',
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url'].'upload_files/'
            .$this->get_query_separator($this->options['script_url'])
            .$this->get_singular_param_name()
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file = new \stdClass();
        $file->name2 = $this->get_file_name($uploaded_file, $name, $size, $type, $error,
            $index, $content_range);
        ...
        if (empty($file->error)) {
          $stmt = $dbConnection->prepare('insert into files_comment
(comment_hash, original_name, file_hash, file_type, file_size, file_ext) values
(:comment_hash, :original_name, :file_hash, :file_type, :file_size, :file_ext)');
$stmt->execute(array(
':comment_hash'	=>$hn,
