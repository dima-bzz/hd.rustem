...
if (validate_user($_SESSION['helpdesk_user_id'], $_SESSION['code'])) {
    ...
    if (!empty($res1)) {
    foreach($res1 as $row) {
$to_text = view_array(get_fio_name_return($row['user_to_id']));
...
window.print();
	    ...
	    <center><h3><?=lang('TICKET_name');?></h3></center>
                ...
                <table>
                    <tbody>
                    <tr>
                        <td style=" padding: 20px; " colspan="2"><?=make_html($row['msg'])?></td>
                    </tr>
                    </tbody>
                </table>
	    ...
	    <div style="padding-top:50px;margin-right:50px;text-align:right;">
	    <?=name_of_user($row['user_init_id'])?>  / _____________________
	    </div>
        <?php
	}
